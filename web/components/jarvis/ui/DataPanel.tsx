  ...
  useEffect(() => {
    ...
    if (title === 'SYSTEM STATUS') {
      ...
      const statusData: DataItem[] = serviceData.services.map(service => ({
        ...
        status: service.status === 'online' ? 'success' : service.status === 'offline' ? 'error' : 'normal',
      }));

      // Add system health indicator
      const allOnline = serviceData.services.every(s => s.status === 'online');
      statusData.unshift({
      ...
      const diagnosticsData: DataItem[] = [
        ...
        },
        {
          ...
          status: metrics.memory > 80 ? 'error' : metrics.memory > 60 ? 'warning' : 'success'
        },
        {
          label: 'CPU TEMP',
