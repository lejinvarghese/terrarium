...
def get_landscape(landscape_id: str) -> dict:
    ...
    if landscape_id not in LANDSCAPES:
        raise ValueError(f"Unknown landscape: {landscape_id}. Available: {list(LANDSCAPES.keys())}")
...
def list_active_landscapes():
    ...
    return {lid: config for lid, config in LANDSCAPES.items() if config.get("active", False)}
